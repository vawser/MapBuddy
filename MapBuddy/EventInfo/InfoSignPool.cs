...
namespace MapBuddy.EventInfo
{
    internal class InfoSignPool
    {
        ...
        string output_dir = "";
        string header = "";
        string combined_file_name = "Global_SignPool";
        string file_format = "csv";

        public InfoSignPool(string path)
        {
        ...
        public void Execute(bool splitByMap)
        {
            ...
            logger.AddToLog($"Written SignPool data to {output_dir}");
            logger.WriteLog();
        }
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Event.SignPool entry in msb.Events.SignPools)
            {
                string line = "";
