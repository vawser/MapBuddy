...
{
    internal class InfoFallPreventionRemoval
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "FallPreventionRemoval";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoFallPreventionRemoval(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.FallPreventionRemoval entry in msb.Regions.FallPreventionRemovals)
            {
                string line = "";
