...
{
    internal class InfoHorseProhibition
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "HorseProhibition";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoHorseProhibition(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.HorseProhibition entry in msb.Regions.HorseProhibitions)
            {
                string line = "";
