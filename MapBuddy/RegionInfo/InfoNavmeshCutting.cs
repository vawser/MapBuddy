...
{
    internal class InfoNavmeshCutting
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "NavmeshCutting";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoNavmeshCutting(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.NavmeshCutting entry in msb.Regions.NavmeshCuttings)
            {
                string line = "";
