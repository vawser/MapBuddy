...
{
    internal class InfoConnection
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "Connection";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoConnection(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.Connection entry in msb.Regions.Connections)
            {
                string line = "";
