...
{
    internal class InfoBuddySummonPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "BuddySummonPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoBuddySummonPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.BuddySummonPoint entry in msb.Regions.BuddySummonPoints)
            {
                string line = "";
