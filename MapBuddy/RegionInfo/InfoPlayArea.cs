...
{
    internal class InfoPlayArea
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "PlayArea";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoPlayArea(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.PlayArea entry in msb.Regions.PlayAreas)
            {
                string line = "";
