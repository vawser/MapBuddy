...
{
    internal class InfoMapNameOverride
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MapNameOverride";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMapNameOverride(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MapNameOverride entry in msb.Regions.MapNameOverrides)
            {
                string line = "";
