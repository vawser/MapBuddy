...
{
    internal class InfoWeatherOverride
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "WeatherOverride";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoWeatherOverride(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.WeatherOverride entry in msb.Regions.WeatherOverrides)
            {
                string line = "";
