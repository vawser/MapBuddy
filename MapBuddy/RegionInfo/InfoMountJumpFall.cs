...
{
    internal class InfoMountJumpFall
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MountJumpFall";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMountJumpFall(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MountJumpFall entry in msb.Regions.MountJumpFalls)
            {
                string line = "";
