...
{
    internal class InfoMufflingPlane
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MufflingPlane";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMufflingPlane(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MufflingPlane entry in msb.Regions.MufflingPlanes)
            {
                string line = "";
