...
{
    internal class InfoEnvironmentMapPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "EnvironmentMapPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoEnvironmentMapPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.EnvironmentMapPoint entry in msb.Regions.EnvironmentMapPoints)
            {
                string line = "";
