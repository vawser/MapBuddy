...
{
    internal class InfoWindSFX
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "WindSFX";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoWindSFX(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.WindSFX entry in msb.Regions.WindSFX)
            {
                string line = "";
