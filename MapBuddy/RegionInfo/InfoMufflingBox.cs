...
{
    internal class InfoMufflingBox
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MufflingBox";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMufflingBox(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MufflingBox entry in msb.Regions.MufflingBoxes)
            {
                string line = "";
