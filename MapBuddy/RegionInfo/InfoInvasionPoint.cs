...
{
    internal class InfoInvasionPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "InvasionPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoInvasionPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.InvasionPoint entry in msb.Regions.InvasionPoints)
            {
                string line = "";
