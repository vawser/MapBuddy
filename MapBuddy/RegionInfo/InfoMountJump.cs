...
{
    internal class InfoMountJump
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MountJump";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMountJump(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MountJump entry in msb.Regions.MountJumps)
            {
                string line = "";
