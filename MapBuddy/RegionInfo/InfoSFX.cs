...
{
    internal class InfoSFX
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "SFX";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoSFX(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.SFX entry in msb.Regions.SFX)
            {
                string line = "";
