...
{
    internal class InfoPatrolRoute
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "PatrolRoute";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoPatrolRoute(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.PatrolRoute entry in msb.Regions.PatrolRoutes)
            {
                string line = "";
