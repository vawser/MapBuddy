...
{
    internal class InfoWindArea
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "WindArea";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoWindArea(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.WindArea entry in msb.Regions.WindAreas)
            {
                string line = "";
