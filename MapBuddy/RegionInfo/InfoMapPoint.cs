...
{
    internal class InfoMapPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MapPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMapPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MapPoint entry in msb.Regions.MapPoints)
            {
                string line = "";
