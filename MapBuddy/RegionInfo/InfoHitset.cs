...
{
    internal class InfoHitset
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "Hitset";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoHitset(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.Hitset entry in msb.Regions.Hitsets)
            {
                string line = "";
