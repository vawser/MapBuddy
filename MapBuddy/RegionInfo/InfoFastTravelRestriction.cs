...
{
    internal class InfoFastTravelRestriction
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "FastTravelRestriction";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoFastTravelRestriction(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.FastTravelRestriction entry in msb.Regions.FastTravelRestriction)
            {
                string line = "";
