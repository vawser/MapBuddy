...
{
    internal class InfoMapPointDiscoveryOverride
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "MapPointDiscoveryOverride";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoMapPointDiscoveryOverride(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.MapPointDiscoveryOverride entry in msb.Regions.MapPointDiscoveryOverrides)
            {
                string line = "";
