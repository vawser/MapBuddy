...
{
    internal class InfoPatrolRoute22
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "PatrolRoute22";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoPatrolRoute22(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.PatrolRoute22 entry in msb.Regions.PatrolRoute22s)
            {
                string line = "";
