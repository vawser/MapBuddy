...
{
    internal class InfoSoundRegion
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "SoundRegion";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoSoundRegion(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.SoundRegion entry in msb.Regions.SoundRegions)
            {
                string line = "";
