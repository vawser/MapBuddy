...
{
    internal class InfoAutoDrawGroupPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "AutoDrawGroupPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoAutoDrawGroupPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.AutoDrawGroupPoint entry in msb.Regions.AutoDrawGroupPoints)
            {
                string line = "";
