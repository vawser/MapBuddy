...
{
    internal class InfoSpawnPoint
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "SpawnPoint";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoSpawnPoint(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.SpawnPoint entry in msb.Regions.SpawnPoints)
            {
                string line = "";
